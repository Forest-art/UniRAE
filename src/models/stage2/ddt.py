"""
DiTwDDTHead - Diffusion Transformer with Dual-head architecture.
...
from .model_utils import (
    ...
    LabelEmbedder, get_2d_sincos_pos_embed, GaussianFourierEmbedding, modulate
)
...
class DDTBlock(nn.Module):
    """
    DDT Block with per-segment modulation.
    """
    def __init__(
        self,
        hidden_size,
        ...
        **block_kwargs
        ...
        from .model_utils import NormAttention
...
class DiTwDDTHead(nn.Module):
    """
    Diffusion Transformer with Dual-head architecture.
    Uses an encoder-decoder structure for better latent diffusion.
    """
    def __init__(
        self,
        input_size: int = 1,
        ...
        self.blocks = nn.ModuleList([
            DDTBlock(self.encoder_hidden_size if i < self.num_encoder_blocks else self.decoder_hidden_size,
    ...
    def initialize_weights(self, xavier_uniform_init: bool = False):
        ...
        # Zero-out adaLN modulation layers in DDT blocks:
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale, cfg_interval=(0, 1)):
        """
        Forward pass of DDT, but also batches the unconditional forward pass for classifier-free guidance.
        """
        half = x[: len(x) // 2]
    ...
    def forward_with_autoguidance(self, x, t, y, cfg_scale, additional_model_forward, cfg_interval=(0, 1)):
        """
        Forward pass of DDT, but also contain the forward pass for the additional model
        """
